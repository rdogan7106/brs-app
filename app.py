...
class MLPredictor:
    ...
    def train(self, df, test_size=0.3):
        """
        Zaman serisi split ile eğitir (lookahead yok).
        İlk %70 train, son %30 test.
        """
...
class Backtester:
    ...
    def run(self, df_1d, kod, initial_capital=10000,
        ...
        for i in range(ml_start, len(feat) - 1):
            ...
            # Stop-loss / Take-profit / sell sinyali (mevcut pozisyon varsa)
            if position > 0:
                exit_reason = None
...
def fetch_yahoo_data(h_kod, interval="1d", range_param="3mo"):
